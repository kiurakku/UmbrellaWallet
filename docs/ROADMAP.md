...
| N.11 | **Nano (XNO) receive and balance** | ✅ **Receive and balance.** SLIP-0010 at `m/44'/165'/0'` (the path Ledger, Trust Wallet and Nault's BIP39 mode use), then Nano's ed25519 with BLAKE2b-512 in place of SHA-512 — pinned stage by stage (seed → private key → public key → address) to the Nano documentation's own test vector. Addresses decoded with their 40-bit BLAKE2b checksum, `xrb_` accepted as the same account. Balance from a choosable node RPC (Nano.to, NanoSLO, SomeNano) with failover; raw amounts (10^30 per XNO, beyond `decimal`) scaled as big integers; the balance shown is pocketed + receivable, because receivable XNO is the account's money. 📅 **Send:** state blocks, signing and proof of work — not started |
